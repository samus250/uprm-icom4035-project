...
public class Memory {
  ...
  /**
   * Copies the current memory word at the address stored in MAR into the
   * internal MODR
   * 
   ...
   */
  public void read() throws DataObjectOutOfBoundsException, InvalidAddressException,
      AddressOutOfRangeException {
    int address = memoryAddressRegister.getIntFromRegister();

    if (address < 0 || address >= mem.length)
      throw new AddressOutOfRangeException("readWord: Address out of range - " + address);

    if (address % Memory.WORDSIZE != 0)
      throw new InvalidAddressException("readWord: Address is not multiple of " + Memory.WORDSIZE);
  ...
  /**
   * Copies the current content of the MIDR data register to the memory area
   * beginning at address MAR
   * 
   * @throws DataObjectOutOfBoundsException
   * @throws InvalidAddressException
   * @throws IndexOutOfBoundsException
   */
  public void write() throws DataObjectOutOfBoundsException, InvalidAddressException,
      AddressOutOfRangeException {
    int address = memoryAddressRegister.getIntFromRegister();
  ...
  /**
   * Copies the specified data to the memory's write register for future
   * recording in memory (when the write operation is performed).
   * 
   * @param data
   */
