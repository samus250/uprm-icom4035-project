...
public class P1MemSystemTester {
  ...
  protected static void testReserveMemory(P1MemSystem ms, int size) {
    System.out.println("\nReserving Memory : " + size + " bytes.");
    int addr = ms.reserveMemory(size);
    if (addr == -1)
      System.out.println("No block available of size >= " + size);
    else
      System.out.println("Block of size " + size + " has been reserved at address " + addr);
  }

  protected static void testFreeMemory(P1MemSystem ms, int addr, int size) {
